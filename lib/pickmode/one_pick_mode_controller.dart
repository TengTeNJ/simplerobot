...
/// 1.0 APP 的捡球模式
class OnePickModeController extends StatefulWidget {
  const OnePickModeController({super.key});

  @override
  State<OnePickModeController> createState() => _OnePickModeControllerState();
}

class _OnePickModeControllerState extends State<OnePickModeController> {
  ...
  Timer? robotRestTimer;

  /// 机器人休息时间倒计时
  int robotRemainTime = 60;

  /// 机器人休息时间倒计时 剩余时间
  bool robotIsReset = false;
  ...
  void getTodayRobotUserTime() async {
    final _list = await DataBaseHelper()
        .getRobotWorkTimeData(kDataBasePickupBallTimeTableName);
    var todayTime = StringUtil.currentTimeString();
    _list.forEach((element) {
      if (element.time == todayTime) {
        setState(() {
    ...
    RobotManager().dataChange = (TCPDataType type) {
      ...
      if (power < 20 && power > 5) {
        if (lowBatteryAlertIsShow == false) {
          ...
          // 低电量弹窗
          TTDialog.robotLowBatteryDialog(context, currentBattery: power,
              () async {
            NavigatorUtil.pop();
          });
        }
      ...
      if (power <= 5 && shutDownAlertIsShow == false) {
        // 提示关机弹窗
      ...
      if (type == TCPDataType.finishOneFlag) {
        // 機器人撿球成功上報
        ...
        robotIsReset = true;
        imageName = 'mode_pause';

        /// 球满了，APP 开始倒计时
        robotRestTimer = Timer.periodic(Duration(milliseconds: 1000), (timer) {
          setState(() {
            robotRemainTime -= 1;
            if (robotRemainTime == 0) {
              robotRestTimer?.cancel();
              robotIsReset = false;
              imageName = 'mode_start';
              robotRemainTime = totalTime;
            }
          });
        });
      } else if (type == TCPDataType.errorInfo) {
        // 异常信息
        var desc = '';
      ...
      } else if (type == TCPDataType.warnInfo) {
        // 告警信息
  ...
  void getBallData() async {
    ...
    var todayTime = StringUtil.currentTimeString();
    var model = PickupBallModel(
        pickupBallNumber: todayPickUpBalls.toString(), time: todayTime);

    if (timeArray.contains(todayTime)) {
      print('数据库有当天的捡球数');
      DataBaseHelper()
          .updateData(kDataBaseTableName, model.toJson(), model.time);
    } else {
  ...
  Future<void> getDBSpeedData() async {
    ...
    Future.delayed(Duration(milliseconds: 200), () {
      ...
      Future.delayed(Duration(milliseconds: 200), () {
        ...
        Future.delayed(Duration(milliseconds: 200), () {
          if (currentGap == 3) {
            // BleSendUtil.setRobotWaitTime(RobotResetGap.five);
            robotRemainTime = 180;
            totalTime = 180;
  ...
  double getMargin() {
    //
    var fontHeight = boundingTextSize(
            context,
            'AUTO',
            TextStyle(
                color: Colors.white,
            ...
            children: [
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          padding: EdgeInsets.only(
                              left: 0, top: 12, bottom: 12, right: 24),
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Center(
                            child: Image(
                              image:
                                  AssetImage('images/control/setting_icon.png'),
                              width: 23,
                            ),
                          ),
                        ),
              ...
              Container(
                ...
                child: ActionDataListView(
                  todayCount: '${todayPickUpBalls}',
                  useMinutes: todayRobotWorkTime,
                  todayCal: todayCal,
                ),
              ),
                  ...
                  ? Container(
                      ...
                      child: GestureDetector(
                        onTap: () {
                          ...
                          setState(() {
                            imageName = imageName == 'mode_start'
                                ? 'mode_pause'
                                : 'mode_start';
                          });
                        },
                        child: Column(
                          children: [
                            ...
                            Image(
                              image:
                                  AssetImage('images/home/${imageName}.apng'),
                              width: 204,
                              height: 204,
                            ),
                            SizedBox(height: 34),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    /// 自由捡球模式
                    Container(
                      margin: const EdgeInsets.only(bottom: 20, left: 78),
                      child: Column(
                        mainAxisSize: MainAxisSize.min,          // 1. 让 Column 自身宽度紧贴子组件
                        crossAxisAlignment: CrossAxisAlignment.center,
                        children: [
                          GestureDetector(
                            onTap: () {
                            ...
                            child: Container(
                              ...
                              alignment: Alignment.center,       // 2. 图片自己再中心一次
                              child: Image.asset(
                                'images/guide/auto.png',
                                width: 26,
                                height: 28,
                              ),
                            ),
                          ),
                          const SizedBox(height: 8),            // 3. 图片与文字间距
                          Constants.regularWhiteTextWidget(
                            "AUTO",
                            16,
                            Constants.selectedModelBgColor,
                          ),
                        ],
                      ),
                    ),
                    /// 切换遥控
                    Container(
                      margin: EdgeInsets.only(bottom: 20, right: 78),
                      child: Column(
                        mainAxisSize: MainAxisSize.min,          // 1. 让 Column 自身宽度紧贴子组件
                        crossAxisAlignment: CrossAxisAlignment.center,
                        children: [
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Center(
                                child: Image(
                                  image: AssetImage('images/guide/remote.png'),
                                  width: 17,
                                  height: 19,
                                ),
                              ),
                          ...
                          const SizedBox(height: 8),            // 3. 图片与文字间距
                          Constants.regularWhiteTextWidget(
                              "Remote", 16, Constants.selectedModelBgColor),
                        ],
                      ),
                    ),
                  ],
